...
if not WindUI then
    game:GetService("StarterGui"):SetCore("SendNotification", {Title="错误", Text="WindUI加载失败，请检查网络", Duration=5})
    return
end
...
if Window.AddButton then
    ...
    Window:AddButton({ Text = "设置", Callback = function() WindUI:Notify({ Title = "提示", Content = "测试版", Duration = 2 }) end })
end
...
local cashToggle = FarmTab:Toggle({ Title = "自动收集现金", Default = false, Callback = function(v) end })
local oilToggle = FarmTab:Toggle({ Title = "自动油桶", Default = false, Callback = function(v) end })
local airdropToggle = FarmTab:Toggle({ Title = "自动空投", Default = false, Callback = function(v) end })
...
MiscTab:Button({ Title = "清除所有板条箱", Callback = function() print("清除板条箱") end })
MiscTab:Button({ Title = "重新加入服务器", Callback = function() print("重新加入") end })
...
task.spawn(function()
    pcall(function()
        ...
        local Settings = {
            ...
            DayNight = false,
            AutoFarmMaster = false, AutoCash = false, AutoOil = false, AutoAirdrop = false, AutoStealCrate = false, AutoSellCrate = false,
        ...
        local gunModConnection = RunService.RenderStepped:Connect(function()
        ...
        cashToggle:SetCallback(function(v) Settings.AutoCash = v; startAutoFarm() end)
        oilToggle:SetCallback(function(v) Settings.AutoOil = v; startAutoFarm() end)
        airdropToggle:SetCallback(function(v) Settings.AutoAirdrop = v; startAutoFarm() end)
        ...
        -- 按钮
        MiscTab:Button({ Title = "清除所有板条箱", Callback = RemoveAllCrates })
        MiscTab:Button({ Title = "重新加入服务器", Callback = RejoinServer })
